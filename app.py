...
def load_and_train():
    ...
    # ── Deteksi & Hapus Outlier dengan IQR (semua fitur selected) ──
    features_outlier = ['adr', 'lead_time']
    for feat in features_outlier:
    ...
    # ── Deteksi & Hapus Outlier dengan IQR ──
    features_outlier = ['lead_time', 'total_stays', 'total_guests',
                        'total_of_special_requests', 'previous_cancellations']
    for feat in features_outlier:
        Q1  = df_clean[feat].quantile(0.25)
        Q3  = df_clean[feat].quantile(0.75)
    ...
    # Ambil ulang X agar konsisten dengan df_clean setelah IQR
    X = df_clean[selected_features].copy()
...
# ═══════════════════════════════════════════════════════
    ...
    with col_result:
        ...
        if predict_btn:
            ...
            best_model_name = clf_df.loc[clf_df['F1-Score'].idxmax(), 'Model']
            probs_best = clf_fitted[best_model_name].predict_proba(inp_sc)[0]
            fig, ax  = plt.subplots(figsize=(6, 2.5))
            bars = ax.barh([f'Cluster 0 ({label_0})', f'Cluster 1 ({label_1})'],
                           probs_best, color=['#2196F3','#FF9800'], edgecolor='black')
            ax.set_xlim(0, 1)
            ax.set_title(f'Probabilitas ({best_model_name})')
