...
# ─────────────────────────────────────────────────────────────
    ...
    with col_out:
        ...
        if predict_btn:
            votes = []
            rows  = []
            input_df     = pd.DataFrame([[adr, lead_time, total_stays, total_guests, special_req, prev_cancel]], columns=FEATURES)
            ...
            if not votes:
                st.error("Tidak ada model yang berhasil melakukan prediksi.")
                st.stop()
            ...
            rf_model = clf_models.get('Random Forest')
            if rf_model:
                probs_rf = rf_model.predict_proba(input_scaled)[0]
                ...
                ax3.set_xlim(0, 1)
                ax3.set_title('Probabilitas Prediksi (Random Forest)')
                ax3.set_xlabel('Probabilitas')
                for bar, p in zip(bars3, probs_rf):
