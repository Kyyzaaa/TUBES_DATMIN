...
# ─────────────────────────────────────────────────────────────
    ...
    with tab_reg:
        ...
        st.dataframe(
            ...
            .highlight_min(subset=['MAE','MSE','RMSE'], color='#c6efce'),
    ...
    with tab_clf:
        ...
        st.dataframe(
            ...
            .highlight_max(subset=['Accuracy','Precision','Recall','F1-Score','AUC-ROC'], color='#c6efce'),
            use_container_width=True
        )
...
# ─────────────────────────────────────────────────────────────
    ...
    if overfit_meta is None:
        ...
        st.dataframe(
            bv_df.set_index('Model').style.applymap(
                lambda v: 'background-color:#d4edda' if '✅' in str(v)
                else ('background-color:#f8d7da' if '❌' in str(v)
                else ('background-color:#fff3cd' if '⚠️' in str(v) else '')),
                subset=['Status']
            ),
            use_container_width=True
        )
