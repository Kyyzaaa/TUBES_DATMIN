"""
Hotel Booking Demand — Streamlit Dashboard
...
<div class="main-header">
  <h1>🏨 Hotel Booking Demand — Dashboard Analitik</h1>
  <p>CRISP-DM Fase 6: Deployment &nbsp;|&nbsp; PLO08-CLO01</p>
</div>
""", unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════
    ...
    for col, cl, label, color in [
        (col1, 0, label_0, '#e3f2fd'),
        (col2, 1, label_1, '#fff3e0')
    ]:
...
# ═══════════════════════════════════════════════════════
    ...
    st.subheader("1️⃣ Train vs Test Score — Model Klasifikasi")
    ...
    st.subheader("2️⃣ Learning Curves")
    st.info("Learning curves dihitung ulang — harap tunggu ±30 detik.")

    @st.cache_data(show_spinner="Menghitung learning curves...")
    def compute_lc(_clf_fitted, _X_tr, _y_tr):
        lc = {}
        for name, model in _clf_fitted.items():
            n = min(15000, _X_tr.shape[0])
            idx = np.random.RandomState(42).choice(_X_tr.shape[0], n, replace=False)
            sz, t_sc, v_sc = learning_curve(
                model, _X_tr[idx], _y_tr.values[idx],
                cv=3, train_sizes=np.linspace(0.1, 1.0, 7),
                scoring='accuracy', n_jobs=-1
            )
            lc[name] = {'sz': sz, 'train': t_sc, 'val': v_sc}
        return lc

    lc_data = compute_lc(clf_fitted, data['X_tr_cs'], data['y_tr_c'])

    fig, axes = plt.subplots(1, 3, figsize=(15, 4))
    fig.suptitle('Learning Curves — Deteksi Overfitting & Underfitting', fontsize=12, fontweight='bold')
    ...
    st.caption("""
    **Interpretasi:** Train ≈ Val dan keduanya tinggi → Fit Baik ✅ |
    Train tinggi, Val rendah, gap besar → Overfitting ❌ |
    Keduanya rendah → Underfitting ⚠️
    """)
