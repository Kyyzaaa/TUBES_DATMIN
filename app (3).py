...
with st.spinner("⚙️ Menyiapkan pipeline data & model, harap tunggu..."):
    ...
    _centers_raw = clust["centers_orig"].tolist()
    _labels_raw  = clust["labels"]

    # ── Pastikan Cluster 0 = Budget (ADR rendah), Cluster 1 = Premium (ADR tinggi) ──
    # ADR ada di indeks 0 pada FEATURES — remap SEBELUM training model
    if _centers_raw[0][0] <= _centers_raw[1][0]:
        centers = _centers_raw
        labels  = _labels_raw
    else:
        # K-Means assign nomor terbalik → swap centroid & flip semua label
        centers = [_centers_raw[1], _centers_raw[0]]
        labels  = np.where(_labels_raw == 0, 1, 0)
...
def get_cluster_label(cl):
    # Cluster 0 selalu Budget / Short-stay, Cluster 1 selalu Premium / Long-stay
